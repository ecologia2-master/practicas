...
### 1.3 Extractos de `r MI_PSEUDONIMO`
...
- Dibuja las isoclinas a mano (usa tus parámetros).
- Haz lectura de tu caso, interpretando según el gráfico de regiones.
...
![](cuatro-casos-competencia-lv.jpg)
![](cuatro-casos-competencia-lv-v2.png)

### 2.D Explicación en detalle

#### 1) Qué son las isoclinas y qué “dicen”

- **Isoclina de $N_1$**: $N_1 + \alpha_{12}N_2 = K_1$.  
  Debajo de esa línea se cumple $N_1+\alpha_{12}N_2<K_1$ ⇒ $dN_1/dt>0$ (1 **aumenta**).  
  Encima, $dN_1/dt<0$ (1 **disminuye**).

- **Isoclina de $N_2$**: $N_2 + \alpha_{21}N_1 = K_2$.  
  Debajo/izquierda de esa línea: $dN_2/dt>0$ (2 **aumenta**).  
  Arriba/derecha: $dN_2/dt<0$ (2 **disminuye**).

> Cada isoclina parte el plano en dos mitades: donde esa especie **crece** o **decrece**.

---

#### 2) “Converge a la intersección → coexistencia”

El **único** punto del interior donde **ambas** derivadas son cero a la vez es el **cruce de isoclinas**.  
Si las flechas del campo (según los signos de $dN_1$ y $dN_2$) apuntan **hacia** ese cruce, **el punto es estable**: las dos poblaciones se acercan a $(N_1^*,N_2^*)$ ⇒ **coexistencia estable**.

**Regla visual rápida**:
- Debajo de **ambas** isoclinas: flecha ↗︎ (suben las dos).  
...
#### 3) “Cae a un eje → exclusión”

Los ejes $N_1=0$ (abajo) y $N_2=0$ (izquierda) representan **extinción** de una especie.

- En el eje $N_2=0$: la ecuación de $N_2$ queda en 0 ⇒ si la trayectoria llega ahí, **2 queda extinta** y 1 evoluciona solo (logístico) hasta $(K_1,0)$.  
- En el eje $N_1=0$: análogo para 1 ⇒ $(0,K_2)$.

> Cuando la trayectoria “**cae a un eje**”, una especie se **va a 0**: **exclusión competitiva**.

---

#### 4) “2 excluye a 1” en el dibujo: por qué

Mira los **interceptos** de las isoclinas en los ejes:

- En el **eje $N_1$** (abajo): compara $K_1$ (corte de la isoclina de 1) con $K_2/\alpha_{21}$ (corte de la de 2).  
- En el **eje $N_2$** (izquierda): compara $K_1/\alpha_{12}$ (de 1) con $K_2$ (de 2).

**Casos geométricos clásicos**:
- **2 excluye a 1** si la isoclina de 2 está **más afuera** que la de 1 en **ambos ejes**:  
  $K_1 < K_2/\alpha_{21}$ **y** $K_1/\alpha_{12} < K_2$.  
  *Intuición*: 2 “tolera” más densidad propia y del otro (intra > inter para 2); las flechas empujan hacia el **eje $N_1=0$** ⇒ $N_1 \to 0$.

- **1 excluye a 2** si ocurre lo contrario (isoclina de 1 más afuera en ambos ejes).

- **Cruce “alternado”** (uno gana en $x$ y el otro en $y$): hay cruce interior.  
  - Si **intra > inter** para ambos ⇒ **coexistencia estable** (cruce atractor).  
  - Si **inter > intra** ⇒ **coexistencia inestable** (efecto de prioridad: según condiciones iniciales, uno u otro excluye).

> **Ojo**: con parámetros positivos las líneas **se cruzan** geométricamente; lo que cambia es si ese cruce es **estable** o **inestable**. **Puede haber exclusión aunque exista cruce** (si el cruce es un separador y las flechas se van a un eje).

---

#### 5) Receta rápida para leer un caso

1. Dibuja los cortes en cada eje ($K_1$ vs $K_2/\alpha_{21}$; $K_1/\alpha_{12}$ vs $K_2$).  
2. Marca regiones con $dN_1/dt>0$ (debajo de su isoclina) y $dN_2/dt>0$ (debajo de la suya).  
3. Añade 3–4 flechas guía por región.  
4. Si las flechas **entran** al cruce ⇒ **coexistencia**.  
   Si **salen** y terminan en un eje ⇒ **exclusión** (la especie del eje opuesto se extingue).
