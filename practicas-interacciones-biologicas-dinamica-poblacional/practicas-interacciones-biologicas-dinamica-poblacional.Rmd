...
```{r prep}
# Repos por defecto (evita el prompt de selección)
if (is.null(getOption("repos")) || getOption("repos")[["CRAN"]] %in% c("@CRAN@", NA)) {
  options(repos = c(CRAN = "https://cloud.r-project.org"))
}

# Paquetes CRAN a asegurar
cran_pkgs <- c("ggplot2", "digest", "deSolve", "patchwork", "devtools")
...
# ecostudy sólo en GitHub
if (!requireNamespace("ecostudy", quietly = TRUE)) {
  # Asegura devtools (por si no se pudo cargar arriba)
  if (!requireNamespace("devtools", quietly = TRUE)) {
    install.packages("devtools", dependencies = TRUE, quiet = TRUE)
  }
  devtools::install_github("jarioksa/ecostudy", upgrade = "never", dependencies = TRUE)
}
